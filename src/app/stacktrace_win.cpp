...
QString straceWin::GetErrorString(DWORD errNum)
{
    if (errNum == 0)
        // no error
        return QString();
    ...
    DWORD size = FormatMessageW(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM,
        NULL, errNum, MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), (LPTSTR)&buf, 0, nullptr);

    QString message =
#ifdef UNICODE
        QString::fromWCharArray(buf, size);
#else
        QString::fromLocal8Bit(buf, size);
#endif
...
QString straceWin::GetLastErrorString()
{
...
* returns true iff 'dir' is an ancestor of 'file'
*/
bool straceWin::makeRelativePath(const QString& dir, QString& file)
{
    QString d = QDir::toNativeSeparators(QDir(dir).canonicalPath());
    QString f = QDir::toNativeSeparators(QFileInfo(file).canonicalFilePath());
...
#endif
const QString straceWin::getBacktrace(HANDLE hProcess, HANDLE hThread)
{
    DWORD MachineType;
    CONTEXT Context;
    ...
    if (SuspendThread(hThread) == (DWORD)-1) {
        //error happened
        QString msg = straceWin::GetLastErrorString();
        qDebug() << "SuspendThread(): " << msg;
    }

    if (!GetThreadContext(hThread, &Context)) {
        //error happened
        QString msg = straceWin::GetLastErrorString();
        qDebug() << "GetThreadContext(): " << msg;
    }
    if(ResumeThread(hThread) == (DWORD)-1) {
        //error happened
        QString msg = straceWin::GetLastErrorString();
        qDebug() << "ResumeThread(): " << msg;
    }
#else
